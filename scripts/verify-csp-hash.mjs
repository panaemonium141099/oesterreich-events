...
async function main() {
  ...
  // 2026-09-03 — this check is INVERTED from what fn-15.6 shipped.
  //
  // It used to assert that style-src-elem CONTAINS 'sha256-<critical css>'.
  // That hash is exactly what broke production: per CSP Level 3, a hash or
  // nonce anywhere in a source list makes the browser IGNORE 'unsafe-inline'
  // for that entire directive, so the directive silently flips to hash-only
  // and every <style> element injected at runtime by a third-party script
  // gets blocked — Google's consent banner, Mapbox GL, Booking.com and
  // GetYourGuide all inject one. The banner rendered as unstyled text with
  // its Material-Icons ligatures showing raw ("verified_user", "close").
  //
  // So we now assert the opposite: style-src-elem must carry 'unsafe-inline'
  // and must NOT carry any hash or nonce. Re-introducing one is only safe
  // together with a per-request nonce on every <style> we and our third
  // parties emit — which is a different piece of work, not a one-line CSP
  // edit. This guard exists to make that regression loud at build time.
  //
  // routes-manifest.json is generated by `next build` and holds the resolved
  // response headers per route.
  ...
  if (existsSync(manifestPath)) {
    try {
      ...
      const broken = [];
      for (const h of cspHeaders) {
        const elemMatch = h.value.match(/style-src-elem[^;]*/);
        if (!elemMatch) {
          broken.push({ ...h, why: 'style-src-elem directive missing entirely' });
          continue;
        }
        const clause = elemMatch[0];
        const offender = clause.match(/'(?:sha(?:256|384|512)-|nonce-)[^']*'/);
        if (offender) {
          broken.push({
            ...h,
            why:
              `carries ${offender[0]} — that silently disables 'unsafe-inline' ` +
              `for the whole directive and blocks every runtime-injected <style>`,
          });
          continue;
        }
        if (!clause.includes("'unsafe-inline'")) {
          broken.push({ ...h, why: "style-src-elem has no 'unsafe-inline'" });
        }
      }
      if (broken.length > 0) {
        console.error(
          `[csp-verify] FATAL: ${broken.length}/${cspHeaders.length} CSP header(s) ` +
            `in routes-manifest.json have an unusable style-src-elem.`,
        );
        for (const h of broken) {
          console.error(`  - source="${h.source}": ${h.why}`);
          console.error(`    csp=${h.value.slice(0, 200)}…`);
        }
        console.error(
          `[csp-verify] Fix next.config.ts headers(): style-src-elem must list ` +
            `'unsafe-inline' and must NOT list a sha256/nonce source. See the ` +
            `comment above styleSrcCommon for why.`,
        );
        process.exit(1);
      }
      console.log(
        `[csp-verify] OK — routes-manifest.json: ${cspHeaders.length} CSP header(s), ` +
          `style-src-elem is 'unsafe-inline' with no hash/nonce shadowing it.`,
      );
    } catch (err) {
      console.warn(
