...
- loadNearbyActivitiesCached analog loadNearbyEventsCached (gemeinde/[slug]/page.tsx:85-116): unstable_cache + bboxAround + haversineKm, Radius via getCityHub(...)?.radiusKm ?? 10.
  ...
  <!-- Updated by plan-sync: fn-18.2 importiert dauerhaft geschlossene POIs mit is_closed=true, aber visible=true (is_closed ist SEPARAT von visible, Migration 20260724120000:94-98). Weil dieser Loader die Basistabelle direkt liest (obige fn-18.1-Notiz), entfaellt der view-seitige Auto-Filter der poi_activities_public (`visible AND NOT is_closed`, Migration :168) -> loadNearbyActivitiesCached muss BEIDE Bedingungen manuell setzen: visible = true AND is_closed = false. Sonst erscheinen geschlossene Aktivitaeten in der Hub-Sektion + im Event-Detail-Cross-Link (Task-2-Spec: "via is_closed von Hub/Sitemap/Suche/Affiliate ausgeschlossen"). -->
