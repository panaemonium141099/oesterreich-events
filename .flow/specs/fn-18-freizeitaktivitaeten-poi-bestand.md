...
- **E6 Prune mit Run-Bookkeeping**: eigene Tabelle `poi_activity_runs` (run_id, started_at, finished_at, regions_ok, regions_failed, is_complete) + pro Row `last_seen_run_id` und `seen_regions text[]`. **„Kompletter Lauf"** = alle 131 Regionen versucht und 0 in regions_failed; `--region`- und `--dry-run`-Läufe zählen nie. Prune: `visible=false` erst, wenn eine Row in den letzten 2 kompletten Läufen nicht gesehen wurde. Damit können Teil-/Fehlläufe niemals fälschlich Rows verstecken.
...
- **E11 Dedup-Fallback unabhängig von der Mirror-GUID-Frage**: zusätzlich zur UNIQUE(source, source_id) bekommt jede Row einen `content_fingerprint` (Hash aus normalisiertem Namen + auf ~50 m gerundeten Koordinaten). Liefern Mirror-Regionen doch unterschiedliche GUIDs, werden Fingerprint-Duplikate deterministisch markiert (`duplicate_of`, Nicht-Kanonische `visible=false`). Das Schema aus Task 1 bleibt damit korrekt, egal wie der Overlap-Report in Task 2 ausgeht.
- **E12 Sitemap-Split JETZT** (kein „ggf."): `src/app/sitemap.xml/route.ts` wird zum **Sitemap-Index** (`<sitemapindex>`), der auf neue Kind-Routen zeigt: `/sitemap-core.xml` (Statisch/Hubs/Themen/Studenten/Venues), `/sitemap-events.xml` (bestehende Event-Logik inkl. MAX_EVENTS-Cap), `/sitemap-activities.xml` (nur indexierbare Aktivitäten, E7-Gate). robots.txt zeigt weiter auf /sitemap.xml; nach Deploy Sitemap in GSC neu einreichen.
...
- [ ] Smart-Suche: „wo kann ich mountaincart fahren" liefert `activityMatches` (eigener Pfad, intentIsEmpty-Fix); Regressionstest belegt unveränderte Event-Queries (future-only)
