/**
 * GET /api/event-reminder/unsubscribe?email=…&event=…&token=… — Abmeldung
 * aus dem Mail-Footer (DSGVO). Token = HMAC(event-reminder-unsub:email:eventId).
 */
...
export async function GET(req: NextRequest) {
  ...
  if (!email || !eventId || !(await verifyReminderToken('unsub', email, eventId, token))) {
    return page('Link ungültig', 'Dieser Abmeldelink ist ungültig oder unvollständig.');
  }
  ...
  const { error } = await admin
    ...
    .eq('event_id', eventId);
  if (error) {
    console.error('[event-reminder/unsubscribe] update failed:', error);
    return page('Etwas ist schiefgegangen', 'Bitte versuche den Link später erneut.');
  }

  return page('Erinnerung abbestellt', 'Für dieses Event bekommst du keine weiteren Mails von uns.');
}
