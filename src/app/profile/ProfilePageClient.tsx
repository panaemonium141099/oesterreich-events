 ...
 * thin server component in `./page.tsx` AFTER a successful
 * `requireUserOrRedirect()` check, so by the time this renders the
 * viewer is guaranteed authenticated server-side. The defensive
 * client-side `!loading && !user → /auth/login` fallback below stays
 * for mid-session logouts in other tabs.
 */
