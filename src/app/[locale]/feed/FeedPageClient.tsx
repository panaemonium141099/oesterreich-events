 ...
 * server component in `./page.tsx` AFTER a successful
 * `requireUserOrRedirect()` check, so by the time this renders the
 * viewer is guaranteed to be authenticated on the server. The
 * defensive client-side `!loading && !user → /auth/login` below stays
 * in place solely to handle mid-session logouts (user signs out in
 * another tab — auth context updates, we bounce them out).
 */
