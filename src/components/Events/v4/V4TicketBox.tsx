...
export function V4TicketBox({
  ...
  // Message-basierte Fassung der früheren badgeLabel()-Logik — die
  // deutschen Werte sind byte-identisch zu den alten Template-Strings.
  const badgeLabel =
    variant === 'match' && artistName ? t('badgeMatch', { artist: artistName })
    : variant === 'lineup' && artistName ? t('badgeLineup', { artist: artistName })
    : t('badgeTicket');
  return (
    <div
    ...
    >
      ...
      <div className="p-[20px_22px_22px]">
        <V4Badge kind={badgeKind}>{badgeLabel}</V4Badge>
        ...
        <div className="mt-2.5 flex gap-2">
          ...
          <a href="#share" className="press-haptic inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-full border border-[var(--v4-hairline-3)] text-[12.5px] font-semibold text-[var(--v4-ink)]" aria-label={t('share')}>
        ...
        <div className="mt-4 pt-3.5 border-t border-[var(--v4-hairline-1)] flex flex-col gap-1.5">
          <p className="text-[11.5px] leading-[1.5] text-[var(--v4-ink-50)] flex items-start gap-2">
            ...
            <span>{t('trustExternal')}</span>
          </p>
          <p className="text-[11.5px] leading-[1.5] text-[var(--v4-ink-50)] flex items-start gap-2">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--v4-go)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{flexShrink:0,marginTop:2}}><polyline points="20 6 9 17 4 12"/></svg>
            <span>{t('trustRedirect')}</span>
          </p>
        </div>
      </div>
    </div>
