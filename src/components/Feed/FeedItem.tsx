  ...
  if (isCompactActivity(activity.type)) {
    return (
      <div className="flex items-center gap-2.5 px-5 py-2.5 mx-4 my-2 rounded-lg bg-white/[0.03]">
        ...
        <p className="text-[13px] text-white/50 flex-1 min-w-0">
          <span className="text-white/60">{username}</span>{' '}
          {caption}
          <span className="text-white/40 ml-1.5">{formatRelativeTime(activity.created_at)}</span>
        </p>
      </div>
    );
  }
    ...
    <article>
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-xs text-white/40 ml-2">{formatRelativeTime(activity.created_at)}</span>
        </div>
        <PostMenu
      ...
      <div className="flex items-center justify-between px-4 py-2">
        <div className="flex items-center gap-4">
          ...
          >
            <svg
              className={`w-6 h-6 transition-colors duration-200 ${liked ? 'text-red-500' : 'text-white/60 hover:text-white/40'}`}
          ...
          >
            <svg className="w-6 h-6 text-white/60 hover:text-white/40 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {hasEvent && (
          ...
          >
            <svg
              className={`w-6 h-6 transition-colors duration-200 ${bookmarked ? 'text-white/90' : 'text-white/60 hover:text-white/40'}`}
      ...
      {/* Like Count */}
      {likeCount > 0 && (
        <p className="px-4 text-sm font-semibold text-white/90">
      ...
      {commentCount > 2 && (
        <Link href={`/feed/${activity.id}`} className="block px-4 mt-1">
          <span className="text-sm text-white/40">Alle {commentCount} Kommentare ansehen</span>
        </Link>
      )}
