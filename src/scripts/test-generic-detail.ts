/**
 * Probe extractor against 15 random gemeinden-generic source_urls — proves
 * extractGem2goDetail's universal layers (JSON-LD, og-meta, regex) work on
 * non-gem2go sites too.
 */
...
async function main() {
  ...
  const { data } = await sb.from('events')
    ...
    .eq('source_name', 'gemeinden-generic')
