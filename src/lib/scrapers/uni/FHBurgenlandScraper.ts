 ...
 * Events overview page. Campuses in Eisenstadt and Pinkafeld.
 * Info days, career events.
 */
export class FHBurgenlandScraper extends UniBaseScraper {
  ...
  protected readonly eventListUrl = 'https://hochschule-burgenland.at/en/about-us/events/overview/';
  ...
  async scrape(): Promise<ScrapedEvent[]> {
    ...
    try {
      await this.rateLimit();
      const deUrl = 'https://hochschule-burgenland.at/ueber-uns/veranstaltungen/uebersicht/';
  ...
  private parseHtml(html: string): ScrapedEvent[] {
    ...
    $('article, .event-item, [class*="event"], .veranstaltung, .news-item, .card').each((_, el) => {
      try {
        const $el = $(el);
