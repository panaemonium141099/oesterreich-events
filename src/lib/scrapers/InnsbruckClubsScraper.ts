...
const CLUBS: ClubConfig[] = [
  ...
  },
  {
    ...
    latitude: 47.2680, longitude: 11.3920,
    postalCode: '6020',
  },
  {
    id: 'die-baeckerei',
  ...
  },
  {
    id: 'weekender',
    name: 'Weekender Club',
    url: 'https://weekender.at/',
    latitude: 47.2610, longitude: 11.3940,
    postalCode: '6020',
  },
];
...
export class InnsbruckClubsScraper extends BaseScraper {
  readonly name = 'innsbruck-clubs';

  async scrape(): Promise<ScrapedEvent[]> {
    this.log(`Starte Innsbruck Clubs Scraping (${CLUBS.length} Clubs)...`);
  ...
  private parseClubHtml(html: string, club: ClubConfig): ScrapedEvent[] {
    ...
    $('script[type="application/ld+json"]').each((_, el) => {
      try {
        ...
        for (const item of items) {
          ...
          const sourceId = `innsbruck-clubs-${club.id}-${slug}`;
    ...
    const selectors = [
      'a[href*="/event"]', 'a[href*="/veranstaltung"]', 'a[href*="/termin"]',
