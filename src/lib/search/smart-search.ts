/**
 * runSmartSearch — die komplette Smart-Suche-Pipeline als aufrufbare
 * Funktion (fn-19 Phase B: Code-Motion aus /api/search/semantic, damit
 * der Concierge-Chat sie als Tool nutzen kann; die Route ist jetzt nur
 * noch eine dünne HTTP-Hülle darüber).
 *
 * Ablauf (MASTERPLAN §6 — KI pro QUERY statt pro Event):
...
/** Wird geworfen, wenn Supabase-Env fehlt oder das Retrieval stirbt —
 *  die Route mappt das auf 503, der Chat-Tool-Pfad auf ein leeres
 *  Ergebnis mit Fehler-Notiz ans Modell. */
...
export interface SmartSearchPayload {
  ...
  matches: CandidateEvent[];
  count: number;
  activityMatches: ActivitySearchMatch[];
}

export interface SmartSearchOptions {
  /** fn-19 Phase B: Chat-Tools erzwingen ihren Pfad explizit, statt ihn
   *  aus der Query zu erraten. Die /api/search/semantic-Route nutzt das
   *  NICHT — ihr Verhalten bleibt exakt wie vorher. */
  forceContentTypes?: ContentType[];
}
...
export async function runSmartSearch(
  ...
  // Anon-Key-Fallback für lokale Dev-Setups ohne Service-Key: es werden
  // ohnehin nur published+public Events gelesen, mit Anon-Key greift
  // zusätzlich RLS — strikt sicherer, nur ggf. weniger Treffer.
  const supabaseKey =
    process.env.SUPABASE_SERVICE_ROLE_KEY ?? process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  if (!supabaseUrl || !supabaseKey) {
    throw new SmartSearchUnavailableError('Server misconfigured');
  }
  ...
  //    Function-Timeout blockieren — nach Ablauf degradiert die Suche in
  ...
  // fn-19 Phase B: Tool-Aufrufe des Chat legen den Pfad explizit fest.
  // NACH Gating/Fallback angewandt, damit weder Heuristik noch LLM das
  // Tool-Kommando überstimmen können.
  if (opts.forceContentTypes && opts.forceContentTypes.length > 0) {
    intent.contentTypes = [...opts.forceContentTypes];
  }
  ...
  try {
    ...
    throw new SmartSearchUnavailableError('search unavailable');
  }

  // 4. Ranken + Payload. `matches`/`count` bleiben event-only (alter
